...
namespace TWCore.Net.HttpServer
{
    ...
    public sealed class SimpleHttpServer : IDisposable
    {
        ...
        /// </summary>
        public event RequestDelegateAsync BeginRequest;
        /// <summary>
        /// Event triggered when the request is finalizing
        /// </summary>
        public event RequestDelegateAsync EndRequest;
        /// <summary>
        ...
        public SimpleHttpServer AddHttpController<T>(HttpMethod method, string url, string action = null) where T : HttpControllerBase
            => AddRouteHandler(method, url, async context =>
            {
                var ctl = Activator.CreateInstance<T>();
                ctl.Context = context;
                if (action is null)
        ...
        /// <returns>SimpleHttpServer instance</returns>
        public SimpleHttpServer AddRouteHandler(HttpMethod method, string url, RequestHandlerAsync handler)
        {
        ...
        /// <param name="handler">Delegate handler</param>
        /// <returns>SimpleHttpServer instance</returns>
        public SimpleHttpServer AddGetHandler(string url, RequestHandlerAsync handler) => AddRouteHandler(HttpMethod.GET, url, handler);
        /// <summary>
        ...
        /// <param name="handler">Delegate handler</param>
        /// <returns>SimpleHttpServer instance</returns>
        public SimpleHttpServer AddPostHandler(string url, RequestHandlerAsync handler) => AddRouteHandler(HttpMethod.POST, url, handler);
        /// <summary>
        ...
        /// <param name="handler">Delegate handler</param>
        /// <returns>SimpleHttpServer instance</returns>
        public SimpleHttpServer AddPutHandler(string url, RequestHandlerAsync handler) => AddRouteHandler(HttpMethod.PUT, url, handler);
        /// <summary>
        ...
        /// <param name="handler">Delegate handler</param>
        /// <returns>SimpleHttpServer instance</returns>
        public SimpleHttpServer AddHeadHandler(string url, RequestHandlerAsync handler) => AddRouteHandler(HttpMethod.HEAD, url, handler);
        /// <summary>
        ...
        /// <param name="handler">Delegate handler</param>
        /// <returns>SimpleHttpServer instance</returns>
        public SimpleHttpServer AddDeleteHandler(string url, RequestHandlerAsync handler) => AddRouteHandler(HttpMethod.DELETE, url, handler);
        #endregion
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private async Task ConnectionReceived(object objClient)
        {
            ...
            try
            {
                ...
                if (!await context.LoadRequestAsync().ConfigureAwait(false))
                {
                ...
                if (BeginRequest != null)
                {
                    ...
                    handled = await BeginRequest(context, cts.Token).ConfigureAwait(false);
                }
                #endregion

                #region Route handler
                var uHandler = RouteHandlers.TryGetValue(context.Request.Method, out var handlers) ? 
                    handlers.FirstOrDefault((r, absPath) => r.Match(absPath), context.Request.Url.AbsolutePath) : null;
                if (uHandler != null)
                {
                    ...
                    if (uHandler.Handler != null)
                        await uHandler.Handler(context).ConfigureAwait(false);
                    handled = true;
                }
                #endregion
                ...
                if (EndRequest != null)
                {
                    ...
                    handled = await EndRequest(context, cts.Token).ConfigureAwait(false);
                }
                #endregion
                ...
                cts.Cancel();
                await context.CloseContextAsync().ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                cts.Cancel();
