...
namespace TWCore.Serialization.RawSerializer
{
    public partial class DeserializersTable
    {
        ...
        public object Deserialize(Stream stream)
        {
            ...
            {
                Stream = stream;
                if (StreamReadByte() != DataBytesDefinition.Start)
                    ThrowFormatException();
                var sByte = StreamReadByte();
                value = ReadValue(sByte);
                if (sByte != DataBytesDefinition.ValueNull)
                {
            ...
            catch (Exception ex)
            {
                if (stream.CanSeek)
                {
                    if (ex.Message?.StartsWith("Unexpected byte", StringComparison.OrdinalIgnoreCase) == true)
                    {
                        ...
                        try
                        {
                            var fileName = "DesError-" + Guid.NewGuid() + ".rawbin";
        ...
        private object ReadValue(byte type)
        {
            ...
            {
                var value = (IRawSerializable)metadata.Descriptor.Activator();
